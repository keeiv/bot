...
class AntiSpam(commands.Cog):
    ...
    async def anti_spam_set(self, ctx, enabled: bool = True, messages_per_window: int = 10, window_seconds: int = 10, action: str = "mute"):
        """設置防炸群功能"""
        try:
            ...
            if hasattr(ctx, 'interaction') and ctx.interaction:
                if not ctx.interaction.response.is_done():
                    await ctx.interaction.response.send_message(f"[失敗] 錯誤: {str(e)}", ephemeral=True)
            else:
    ...
    async def on_message(self, message: discord.Message):
        ...
        # 檢查是否為垃圾訊息
        is_spam, message_count = self.anti_spam_manager.check_spam(message.guild.id, message.author.id)
        ...
        if is_spam:
            ...
            try:
                if settings['action'] == 'mute':
                    # 禁言 1 小時
                    await message.author.timeout(timedelta(hours=1), reason="防炸群: 訊息流量被警告")
                elif settings['action'] == 'delete':
                    # 刪除最近 10 條訊息
                    try:
                        deleted = await message.channel.purge(limit=10, check=lambda m: m.author == message.author)
                    except:
                        # 如果不能刪除所有，就算了
                        pass
            except Exception as e:
