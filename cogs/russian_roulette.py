...
class RussianRoulette(commands.Cog):
    ...
    class GameView(ui.View):
        ...
        async def pull_trigger(self, interaction: discord.Interaction, button: ui.Button):
            ...
            if is_hit:
                ...
                # 檢查是否結束遊戲
                if self.game.round >= self.game.max_rounds or self.game.player1_chips <= 0 or self.game.player2_chips <= 0:
        ...
        async def use_item(self, interaction: discord.Interaction, button: ui.Button):
            ...
            for i, item in enumerate(items, 1):
                embed.add_field(name=f"{i}. {item}", value=self._get_item_description(item), inline=False)
        ...
        async def _end_game(self):
            ...
            winner = self.game.player1 if self.game.player1_chips > self.game.player2_chips else self.game.player2
    ...
    class ItemSelectView(ui.View):
        ...
        def __init__(self, game: 'RouletteGame', cog: 'RussianRoulette', items: List[str]):
        ...
        async def _use_item(self, interaction: discord.Interaction, item: str):
            ...
            if item == "透視眼鏡":
                ...
                if self.game.used_force_redirect[interaction.user.id]:
                    await interaction.response.send_message("此道具每場遊戲限用一次", ephemeral=True)
    ...
    async def start_roulette(self, interaction: discord.Interaction, opponent: discord.Member):
        """開始俄羅斯輪盤遊戲"""
...
class GameInviteView(ui.View):
    ...
    def __init__(self, game: RussianRoulette.RouletteGame, cog: RussianRoulette, opponent: discord.Member):
        super().__init__(timeout=180)  # 3分鐘超時
        self.game = game
        self.cog = cog
