...
from src.utils.api_optimizer import get_api_optimizer, connection_manager, performance_monitor
...
class SystemMaintenance(commands.Cog):
    ...
    async def system_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            memory_color = discord.Color.green() if health['memory_percent'] < self.memory_threshold else discord.Color.red()
            cpu_color = discord.Color.green() if health['cpu_percent'] < self.cpu_threshold else discord.Color.red()
            ...
            if performance_stats:
                avg_response_time = sum(stats['avg'] for stats in performance_stats.values()) / len(performance_stats)
                embed.add_field(
    ...
    @app_commands.describe(
        pattern="清理模式 (channel/user/guild/all)",
        confirm="確認清理操作"
    )
    async def clear_cache(self, interaction: discord.Interaction, pattern: str = "all", confirm: bool = False):
        if not confirm:
            await interaction.response.send_message("Please set confirm=True to clear cache", ephemeral=True)
            return
        ...
        try:
            ...
            if pattern == "all":
                ...
                await interaction.followup.send("Invalid pattern. Use: channel, user, guild, or all")
                return
